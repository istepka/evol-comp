...
# Parse the output.txt file with new format
    ...
    for line in lines:
        ...
        if line.startswith('Problem instance:'):
            if result_dict and current_method and current_instance:
                ...
                result_dict = {}
            current_instance = line[len('Problem instance:'):].strip()
        elif line.startswith('Search type:'):
            search_type = line[len('Search type:'):].strip()
        elif line.startswith('Initial solution type:'):
            initial_solution_type = line[len('Initial solution type:'):].strip()
        elif line.startswith('Inter neighbourhood type:'):
            inter_neighbourhood_type = line[len('Inter neighbourhood type:'):].strip()
            # Construct method name
            current_method = f"{search_type}_{initial_solution_type}_{inter_neighbourhood_type}"
        elif line.startswith('Best cost:'):
            result_dict['best_cost'] = float(line[len('Best cost:'):].strip())
        elif line.startswith('Worst cost:'):
            result_dict['worst_cost'] = float(line[len('Worst cost:'):].strip())
        elif line.startswith('Average cost:'):
            result_dict['average_cost'] = float(line[len('Average cost:'):].strip())
        elif line.startswith('Best time:'):
            result_dict['best_time'] = float(line[len('Best time:'):].strip())
        elif line.startswith('Worst time:'):
            result_dict['worst_time'] = float(line[len('Worst time:'):].strip())
        elif line.startswith('Average time:'):
            result_dict['average_time'] = float(line[len('Average time:'):].strip())
        elif line.startswith('Best solution:'):
            solution_str = line[len('Best solution:'):].strip()
            result_dict['best_solution'] = [int(s) for s in solution_str.split()]
        elif line.startswith('Writing to:'):
            # End of this block, save the current result_dict
            if result_dict and current_method and current_instance:
                if current_instance not in all_results:
                    all_results[current_instance] = {}
                all_results[current_instance][current_method] = result_dict
                result_dict = {}
            # Optionally, you can parse the filename if needed
