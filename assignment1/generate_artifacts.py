...
RESULTS_EXPORT_DIR = Path('exported_results')  # New directory for exported CSVs
PLOTS_DIR = Path('plots')
LATEX_TABLE_FILE = 'results_table.tex'

# Define Instances and Methods
INSTANCES = ['TSPA', 'TSPB']
METHODS = ['greedy', 'nn1', 'nn2', 'random']
...
def read_node_data(instance: str) -> pd.DataFrame:
    ...
    if not file_path.exists():
        raise FileNotFoundError(f"Data file for instance {instance} not found at {file_path}")
        ...
        for line in lines:
            ...
            if line.startswith('Average cost:'):
                ...
                try:
                    ...
                    logging.debug(f"Parsed average_cost: {result['average_cost']} from {file_path}")
                ...
                try:
                    ...
                    result['worst_cost'] = float(worst_cost_str)  # Using float to accommodate potential floats
                    worst_solution_str = parts[1].strip()
                    result['worst_solution'] = [int(idx) for idx in worst_solution_str.split()]
                    logging.debug(f"Parsed worst_cost: {result['worst_cost']} and worst_solution: {result['worst_solution']} from {file_path}")
                except (IndexError, ValueError) as e:
                    logging.error(f"Error parsing worst_cost or worst_solution in {file_path}: {e}")
                ...
                try:
                    ...
                    result['best_cost'] = float(best_cost_str)  # Using float to accommodate potential floats
                    best_solution_str = parts[1].strip()
                    result['best_solution'] = [int(idx) for idx in best_solution_str.split()]
                    logging.debug(f"Parsed best_cost: {result['best_cost']} and best_solution: {result['best_solution']} from {file_path}")
                except (IndexError, ValueError) as e:
                    logging.error(f"Error parsing best_cost or best_solution in {file_path}: {e}")
...
def collect_results(instances: List[str], methods: List[str]) -> Dict[str, Dict[str, Dict[str, Any]]]:
    all_results = {instance: {method: {} for method in methods} for instance in instances}
    ...
    for method in methods:
        for instance in instances:
            ...
            if file_path.exists():
                ...
                logging.info(f"Parsed results for method '{method}' on instance '{instance}'.")
            else:
    ...
    with open(output_file, 'w') as f:
        ...
        for instance in instances:
            for method in methods:
                ...
                if res:
                    best_cost = f"{res['best_cost']:.2f}" if res.get('best_cost') is not None else "N/A"
                    worst_cost = f"{res['worst_cost']:.2f}" if res.get('worst_cost') is not None else "N/A"
                    average_cost = f"{res['average_cost']:.2f}" if res.get('average_cost') is not None else "N/A"
                    f.write(f"{instance} & {method} & {best_cost} & {worst_cost} & {average_cost} \\\\\n")
                else:
...
def visualize_solution(instance: str, method: str, solution: List[int], node_data: pd.DataFrame, output_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        logging.debug(f"Selected nodes for {method} on {instance}:\n{selected_nodes.head()}")

        # Plot all nodes
        plt.scatter(node_data['x'], node_data['y'], color='lightgrey', label='Unselected Nodes', alpha=0.6)
        ...
        plt.plot(selected_nodes['x'], selected_nodes['y'], 'r-', linewidth=1, label='Hamiltonian Cycle')
...
def main():
    ...
    generate_latex_tables(all_results, INSTANCES, METHODS, Path(LATEX_TABLE_FILE))

    # Step 4: Generate plots and export best solutions to CSV
    PLOTS_DIR.mkdir(parents=True, exist_ok=True)
    RESULTS_EXPORT_DIR.mkdir(parents=True, exist_ok=True)  # Ensure export directory exists
    for instance in INSTANCES:
        for method in METHODS:
            res = all_results[instance][method]
            if res and 'best_solution' in res and res['best_solution']:
                ...
                # Generate plot
                plot_path = visualize_solution(instance, method, best_solution, node_data, PLOTS_DIR)
                if plot_path is None:
                    logging.warning(f"Plot not generated for {method} on {instance} due to invalid solution.")

                # Export best solution to CSV
                csv_path = export_best_solution_to_csv(instance, method, best_solution, node_data, RESULTS_EXPORT_DIR)
                if csv_path is None:
                    logging.warning(f"CSV export not performed for {method} on {instance} due to invalid solution.")
            else:
                logging.warning(f"No valid best_solution available for {method} on {instance}, skipping plot and CSV export.")
